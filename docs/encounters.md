...
`gift` 的主判定仍优先使用脚本命令解析结果；但当前 ROM 的部分赠送脚本会先经过简化解析器暂未覆盖的流程命令，导致 `0x79 givepokemon` 位于未解析段内。为避免整类赠送漏扫，`0x79` 还允许通过结构校验进入 Encounter：命令长度完整、`species/level/item` 在合法范围内，且命令结束后的下一字节是已知脚本命令。这个规则只用于 `givepokemon`；`giveegg` 原始扫描误报较多，仍要求命令偏移能被脚本解析器走到。
...
| `385 漂浮泡泡` | 119号道路，赠送 Lv25；120号道路，草丛 Lv25-27 |
