...
当前 ROM 仍沿用 Emerald 的 `MapHeader`/`MapLayout`/`MapEvents` 结构，但实际使用的是扩展后的地图组表。

已定位：

- active `gMapGroups` pointer table offset：`0xE8C020`，GBA pointer `0x08E8C020`
- 原始 Emerald `gMapGroups` 仍存在于 `0x486578`，但当前 ROM 中未找到代码引用；`0x08E8C020` 在代码区有引用，视为活动表
- 地图组：`38` 组；`0..33` 与 pret/pokeemerald 的 `data/maps/map_groups.json` 顺序一致，`34..37` 为当前 ROM 扩展组
- 有效 `MapHeader`：`702` 个；其中标准组 `518` 个，扩展组 `184` 个
- 当前验证到的扩展组有效数量：`group 34 = 40`、`group 35 = 46`、`group 36 = 59`、`group 37 = 39`
- `RegionMapEntry` table offset：`0x5A1480`，每项 `8` bytes；地图显示名文本池从 `0x5A0B10` 附近开始
- 当前 ROM 中有 `115` 个地图有关联野生 Encounter；Encounter 本身不在 `MapHeader` 内，而是通过 `(map_group, map_number)` 从野生 Encounter header 表反查

地图实体在工具内建议建模为：

| 字段 | 来源 | 说明 |
| --- | --- | --- |
| `map_group` | `gMapGroups` 第几组 | 地图组 ID |
| `map_number` | 组内第几个 header pointer | 地图号 |
| `map_key` | Emerald map group 顺序或扩展组编号 | 例如 `Route103`；扩展组可先记为 `group34_map0` 形式 |
| `name` | `MapHeader.regionMapSectionId -> RegionMapEntry.name` | 游戏中显示的地图名，例如 `103号道路`、`生命之殿` |
| `region_map_section_id` | `MapHeader +0x14` | 地图名/地区图位置索引 |
| `header_offset` | header pointer 转 ROM offset | 当前地图 header 的 ROM offset |
| `layout` | `MapHeader.mapLayout` | 地图尺寸、格子数据、tileset |
| `events` | `MapHeader.events` | NPC、warp、坐标触发、背景事件 |
| `scripts` | `MapHeader.mapScripts` | 地图脚本入口；为 `0` 时没有脚本 |
| `connections` | `MapHeader.connections` | 上下左右等相邻地图连接；为 `0` 时没有连接 |
| `music` | `MapHeader +0x10` | 背景音乐 ID |
| `map_layout_id` | `MapHeader +0x12` | layout ID |
| `cave` | `MapHeader +0x15` | 洞窟/闪光等环境标记 |
| `weather` | `MapHeader +0x16` | 天气 ID |
| `map_type` | `MapHeader +0x17` | 城镇/道路/室内/水下等类型 |
| `allow_cycling` | `MapHeader +0x1A bit0` | 是否允许骑车 |
| `allow_escaping` | `MapHeader +0x1A bit1` | 是否允许穿洞绳/挖洞逃离 |
| `allow_running` | `MapHeader +0x1A bit2` | 是否允许跑步 |
| `show_map_name` | `MapHeader +0x1A bit3..7` | 进图时是否显示地图名相关位 |
| `battle_type` | `MapHeader +0x1B` | 战斗背景/战斗类型 |
| `encounters` | 野生 Encounter header 表 | 按 `(map_group, map_number)` 关联到草丛/冲浪/碎岩/钓鱼列表 |

`MapHeader` 结构，大小 `0x1C`：

| 偏移 | 字段 |
| --- | --- |
| `+0x00` | `MapLayout *mapLayout` |
| `+0x04` | `MapEvents *events` |
| `+0x08` | `u8 *mapScripts` |
| `+0x0C` | `MapConnections *connections` |
| `+0x10` | `music u16` |
...
| `+0x17` | `mapType u8` |
| `+0x18..+0x19` | padding |
| `+0x1A` | flags bitfield：cycling / escaping / running / show map name |
| `+0x1B` | `battleType u8` |

`MapLayout` 结构，大小 `0x18`：

| 偏移 | 字段 | 说明 |
| --- | --- | --- |
| `+0x00` | `width s32` | 地图宽度，单位为 metatile |
| `+0x04` | `height s32` | 地图高度，单位为 metatile |
| `+0x08` | `border u16 *` | 边界块数据 |
| `+0x0C` | `map u16 *` | 地图格子数据 |
| `+0x10` | `primaryTileset *` | 主 tileset |
| `+0x14` | `secondaryTileset *` | 副 tileset |
...
| `+0x04` | `ObjectEventTemplate *objectEvents` |
| `+0x08` | `WarpEvent *warps` |
| `+0x0C` | `CoordEvent *coordEvents` |
| `+0x10` | `BgEvent *bgEvents` |

事件子结构：

| 类型 | 大小 | 关键字段 |
| --- | --- | --- |
| `ObjectEventTemplate` | `0x18` | `localId`、`graphicsId`、`x/y`、`elevation`、`movementType`、移动范围、trainer 字段、`script pointer`、`flagId` |
| `WarpEvent` | `0x08` | `x/y`、`elevation`、`warpId`、目标 `mapNum/mapGroup` |
| `CoordEvent` | `0x10` | `x/y`、`elevation`、`trigger`、`index`、`script pointer` |
| `BgEvent` | `0x0C` | `x/y`、`elevation`、`kind`、脚本/隐藏道具/秘密基地 ID union |
| `MapConnections` | `0x08 + count * 0x0C` | `count`、连接数组；每条连接含方向、offset、目标 `mapGroup/mapNum` |

`RegionMapEntry` 结构，大小 `0x08`：

| 偏移 | 字段 | 说明 |
| --- | --- | --- |
| `+0x00` | `name pointer` | 地图显示名文本，当前 ROM 使用内置中文 GBA 字码 |
| `+0x04` | `x u8` | 地区图 X |
| `+0x05` | `y u8` | 地区图 Y |
| `+0x06` | `width u8` | 地区图宽度 |
| `+0x07` | `height u8` | 地区图高度 |

当前 ROM 全量地图统计：

- 含 NPC/object events 的地图：`608`，object event 总数 `3694`
- 含 warp 的地图：`603`，warp 总数 `1801`
- 含 coord events 的地图：`105`，coord event 总数 `520`
- 含 bg events 的地图：`173`，bg event 总数 `810`
- 含 connections 的地图：`95`，connection 总数 `202`
- 野生 Encounter 聚合：`315` 个 species、`872` 条 species/location/method 聚合记录，覆盖 `115` 个地图

样例：

| map group-number | name | header | mapsec | size | events | connections | music | layout | map_type | weather |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `0-18` | `103号道路` | `0x4826B0` | `18` | `80x22` | object `19` / warp `1` / coord `0` / bg `1` | `3` | `359` | `19` | `3` | `2` |
| `24-11` | `橙华森林` | `0x483FF4` | `59` | `48x44` | object `12` / warp `8` / coord `3` / bg `6` | `0` | `366` | `135` | `3` | `11` |
| `34-0` | `生命之殿` | `0xE4708C` | `138` | `19x20` | object `3` / warp `3` / coord `3` / bg `0` | `0` | `555` | `442` | `10` | `0` |
| `37-38` | `不存在之所` | `0xE71F2C` | `172` | `12x20` | object `0` / warp `2` / coord `0` / bg `6` | `0` | `443` | `630` | `10` | `11` |

结构参照：pret/pokeemerald 的 `include/global.fieldmap.h` 和 `data/maps/map_groups.json`。当前 ROM 的活动表、扩展组和中文 `RegionMapEntry` 为本 ROM 实测结果。

### ROM 野生 Encounter 表

当前 ROM 的活动野生遭遇 header 表已定位：

- active header table offset：`0xEA2D34`
- legacy header table offset：`0x552D48`，内容较旧且缺少扩展地图；解析时按连续有效 header 数选择活动表
...
wild list 每项 4 bytes：`min_level u8`、`max_level u8`、`species u16`。槽位数量分别为草丛 `12`、冲浪 `5`、碎岩 `5`、钓鱼 `10`。当前 UI 的 Encounter `rate` 是按槽位权重聚合后的 species 概率，分母为 `100`，同一地图同一方式下加和应为 `100%`；原始 map encounter 触发率保留为 `encounter_rate`。

land/water 指针的 UI 方式名会按当前地图头修正：`MapHeader +0x17 map_type == 4` 的 land 表显示为“山洞”；水下 map key 或 `map_type == 5` 的 water 表显示为“潜水”。权重和槽位数量仍按 ROM 野生表原始类型解析。
