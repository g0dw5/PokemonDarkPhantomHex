...
wild list 每项 4 bytes：`min_level u8`、`max_level u8`、`species u16`。槽位数量分别为草丛 `12`、冲浪 `5`、碎岩 `5`、钓鱼 `10`。当前 UI 会按 species 聚合为地图名、方式、等级范围和遭遇率；遭遇率字段分母为 `100`，UI 展示为百分比。地图名映射参考 pret/pokeemerald 的 `data/maps/map_groups.json`，同时在数据里保留 `location_id` 形式的原始地图组/地图号。
