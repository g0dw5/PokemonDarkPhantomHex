...
- 额外 20 bytes 是队伍实时字段；当前代码读取 `level`、`current_hp`、`max_hp`、`attack`、`defense`、`speed`、`sp_attack`、`sp_defense`。
- 当前 Web 只写队伍额外字段中的 `level` 和 `current_hp`；能力值不会重算写入。
...
- 盒子记录没有队伍的额外实时字段；等级、当前 HP、能力值不直接存为独立字段。当前 UI 用 `PC` 标记盒子等级显示，并在生成招式约束时按 100 级处理。
...
- 盒子宝可梦没有直接存储等级字段，当前 UI 生成约束时按 100 级处理。
